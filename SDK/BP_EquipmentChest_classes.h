// BlueprintGeneratedClass BP_EquipmentChest.BP_EquipmentChest_C
// Size: 0x5a8 (Inherited: 0x5a0)
struct ABP_EquipmentChest_C : APossessionsChest {
	struct UHitRegSnapshotRedirectImpactToReplicatedMovementAttachParentComponent* HitRegSnapshotRedirectImpactToReplicatedMovementAttachParent; // 0x5a0(0x08)

	void UserConstructionScript(); // Function BP_EquipmentChest.BP_EquipmentChest_C.UserConstructionScript // Event|Public|BlueprintCallable|BlueprintEvent // @ game+0x188c2f0
};

